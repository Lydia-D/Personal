...
% function answer = prelim_q3_1()
    ...
    syms X Y Z
    % z = c/a(x-x1) + z1 
    Xplane = solve((X-Sat_ECEF_global(1,1))*(Landunit_ECEF_sat(3,1)/Landunit_ECEF_sat(1,1))+Sat_ECEF_global(3,1)-Z == 0,X)
...
% z = c/b(y-y1) + z1     
     Yplane =  solve((Y-Sat_ECEF_global(2,1))*(Landunit_ECEF_sat(3,1)/Landunit_ECEF_sat(2,1))+Sat_ECEF_global(3,1)-Z == 0,Y)
   
% sphere: 0= x^2+y^2+z^2 - r^2  
     subs(X,Xplane)
     subs(Y,Yplane)
     sphere = solve( X^2+Y^2+Z^2-r_earth^2 == 0,Z)
