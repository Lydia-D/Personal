...
map = generateGridclear(mapDim,cellDim);
...
storemap = map;
load('sitesOfInterest.mat')  % output x and y
x = [startPos(1);goalPos(1);x];
y = [startPos(2),goalPos(2),y];  % add start and end
for i_start = 1:1:length(x)
    startPos = [x(i_start),y(i_start)];
for i_end = i_start+1:1:length(x)
    goalPos = [x(i_end),y(i_end)];
    map = storemap;
...
% else
    ...
    while map(goalNode) == 4 || mapTraversability(goalNode) == inf
       % go to closest point outside of obstacle? - use findneighbours?
       [neighbours] = getNeighbours(goalCell(1),goalCell(2),map,mapHeight,mapWidth,conn,mapTraversability);
        if isempty(neighbours)
            % move goal node closer to start node?
            walk = (startCell-goalCell)./abs(startCell-goalCell);
            
            [goalNodesubx,goalNodesuby] = ind2sub(size(map),goalNode);
            goalNodesubx = goalNodesubx + walk(1);
            goalNodesuby = goalNodesuby + walk(2);
            goalNode = sub2ind(size(map),goalNodesubx,goalNodesuby);
        else
            goalNode = ind2sub(size(map),neighbours(1));
        
            % store into main arrays
            change = cell2pos([goalNodesubx,goalNodesuby],cellDim,mapDim);
            x(i_start+i_end) = change(2); % swapped because fliplr?
            y(i_start+i_end) = change(1); 
        end
    end
...
%         aStarBody_p(currentNode,g,f,parent,H,heuristic,map,mapTraversability,neighbours,neighbourInd);
    ...
    % saving stuff
    Sites{i_start}{i_end}.cost = cost;
    Sites{i_start}{i_end}.route = route;
end
end
